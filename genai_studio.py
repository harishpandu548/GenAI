...
.card {
    ...
    padding:1.5rem 1.2rem; text-align:center; min-height:170px;
...
.card-title { font-size:1rem; font-weight:700; color:#ffffff; margin-bottom:0.4rem; }
...
# ── Session state ─────────────────────────────────────────────────────────────
for key, default in [("page", "home"), ("research_runs", []),
                     ("rag_ready", False), ("chat_history", [])]:
...
# ── Shared helpers (used by Research page) ───────────────────────────────────
...
def render_trace(messages):
    for msg in messages:
        n, content = type(msg).__name__, str(msg.content) if msg.content else ""
        if "Human" in n:
...
def show_home():
    ...
    with c1:
        ...
        <div class="card">
            ...
            <div class="card-desc">4 agents that search the web, read sources, write a report and critique it</div>
        </div>
        """, unsafe_allow_html=True)
...
def show_research():
    ...
    import agents as ag  # MultiAIAgentSystemProject/agents.py via sys.path
    ...
    if submitted and topic.strip():
        ...
        with st.status("📖  Step 2 · Reader Agent working…", expanded=True) as s:
            st.caption("Extracting content from top source…")
        ...
        st.download_button("↓ Export report", data["report"], "report.txt", "text/plain",
                           key=f"dl_r_{len(st.session_state.research_runs)}")
    ...
    if st.session_state.research_runs:
        ...
        for i, run in enumerate(reversed(st.session_state.research_runs)):
            ...
            with st.expander(f"📋 {run['topic']}"):
                d = run["data"]
                srcs2 = parse_sources(d.get("_raw", ""))
                if srcs2:
                    st.markdown(src_html(srcs2), unsafe_allow_html=True)
                st.markdown(d.get("report", ""))
                st.download_button("↓ Export", d["report"], "report.txt", "text/plain",
                                   key=f"dl_hist_{real}")
                sc = get_score(d.get("feedback", ""))
                if sc:
                    st.markdown(f'<span class="score">⭐ {sc}</span>', unsafe_allow_html=True)
...
def show_rag():
        ...
        if st.button("⚙️ Process PDF", use_container_width=True):
            with st.spinner("Processing PDF and building vector store…"):
                ...
                chunks = RecursiveCharacterTextSplitter(
                    chunk_size=100, chunk_overlap=30).split_documents(docs)
    ...
    if st.session_state.rag_ready:
        ...
        with st.form("rag_form", clear_on_submit=True):
            query = st.text_input("", placeholder="Ask a question about your PDF…",
                                  label_visibility="collapsed")
        ...
        if ask and query.strip():
            with st.spinner("Searching document…"):
                ...
                vs = _Chroma(persist_directory="chroma_db_studio",
...
# ══════════════════════════════════════════════════════════════════════════════
# PAGE 4 — CITY ASSISTANT
# ══════════════════════════════════════════════════════════════════════════════
def show_city():
    back_button()
    st.markdown('<span class="brand">City Assistant</span>', unsafe_allow_html=True)
    st.caption("Ask about weather, news or anything about any city — powered by AI agents")
    st.divider()

    with st.form("city_form", clear_on_submit=True):
        query = st.text_input("", placeholder="e.g. What's the weather and latest news in Mumbai?",
                              label_visibility="collapsed")
        _, col = st.columns([4, 1])
        with col:
    ...
    if st.session_state.chat_history:
        st.divider()
        st.markdown(f"**{len(st.session_state.chat_history)} previous question(s)**")
